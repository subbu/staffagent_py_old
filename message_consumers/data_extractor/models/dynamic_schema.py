    ...
    def create_dynamic_schema(fields_data):
        ...
        def create_field_validator(field_name, field_info):
            def validate_field(v):
                if not isinstance(v, field_info.get('type', Any)):
                    raise ValueError(f"{field_name} must be of type {field_info.get('type', Any)}")
                if 'fields' in field_info:
                    for item in v:
                        for sub_field_name, sub_field_info in field_info['fields'].items():
                            ...
                            if sub_field_value is not None and not isinstance(sub_field_value, sub_field_info.get('type', Any)):
                                raise ValueError(f"{sub_field_name} in {field_name} must be of type {sub_field_info.get('type', Any)}")
                return v
